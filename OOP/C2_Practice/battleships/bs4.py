...
class Game(Frame):
    ...
    def _add_ship_panels(self):
        ...
        self._my_grid_frame._ship_panel.place(
            x=self.X_PADDING, y=self.Y_PADDING * 4)

        for ship in Ship.SHORT_NAMES:
            self._my_grid_frame._ship_panel._ship_buttons[ship].config(
        ...
        self._my_grid_frame._ship_war_panel.place(
            x=self.X_PADDING, y=self.Y_PADDING * 2)
        ##################################################################

        ###################### ShipWarPanel for Adversary ################
        self._their_grid_frame._ship_panel = ShipPanel(self)
        self._their_grid_frame._ship_panel.place(
            x=self._my_grid.size * 2 + self.X_PADDING * 3 + self.SHIP_PANEL_WIDTH, y=self.Y_PADDING * 4)
        ##################################################################
    ...
    def process_state(self):
        ...
        if self._state == self.PLACING:
            self.config(width=self.X_PADDING * 3 + self._my_grid.size +
                        self.SHIP_PANEL_WIDTH + self._my_grid_frame._staging_panel.CANVAS_WIDTH)
            # show staging panel
            self._my_grid_frame._staging_panel.pack_ui()
            self._my_grid_frame._staging_panel.lift(
                aboveThis=self._their_grid_frame)
            ...
            self._my_grid_frame._ship_panel.lift(
                aboveThis=self._my_grid_frame._ship_war_panel)

            # allow the AI to place ships
            self.ai.place_ships()
        elif self._state == self.PLAYING:
            self.config(width=self.X_PADDING * 4 +
                        self._my_grid.size * 2 + self.SHIP_PANEL_WIDTH * 3)
            ...
            self._my_grid_frame._ship_war_panel.lift(
                aboveThis=self._my_grid_frame._ship_panel)

            # show opponent's grid
            self._their_grid_frame.lift(
                aboveThis=self._my_grid_frame._staging_panel)
    ...
    def _process_ai_shot(self):
        ...
        if result == Ship.SUNK:
            self._set_ship_sunk(
                self._my_grid._model.get_sunk_ship(*shot).get_short_name())
    ...
    def _add_grids(self):
        ...
        self._my_grid_frame.place(
            x=self.X_PADDING + self.SHIP_PANEL_WIDTH, y=self.Y_PADDING)
        ...
        self._their_grid_frame.place(
            x=self._my_grid.size + self.X_PADDING * 2 + self.SHIP_PANEL_WIDTH, y=self.Y_PADDING)
        self._their_grid_label = Label(
            self._their_grid_frame, text="Opponent's Grid")
        self._their_grid_label.pack()
    ...
    def reset_closure(self, x, y):
        ...
        def f(event): return self.add_staged_ship(x, y, c)
    ...
    def add_staged_ship(self, x, y, callback):
        ...
        if s is not None:
            self._my_grid.add_ship(
                x, y, s.get_short_name(), s.is_vertical(), callback)
    ...
    def auto_place(self):
        ...
        for ship in ships:
            ...
            self._my_grid.add_ship(*ship.coords(), ship=ship._type,
                                   vertical=ship._vertical == "v", callback=self.get_add_ship_callback())
        self.unselect_ship()
    ...
    def _make_buttons(self):
        ...
        self._play_game_button = Button(
            button_frame, text="Play", command=self.play_game)
        self._play_game_button.pack(side=LEFT, padx=5, pady=5)

        self._my_grid_frame._autoplace_button = Button(
            button_frame, text="Auto-place ships", command=self.auto_place)
