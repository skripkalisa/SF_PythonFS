...
# y axis is labeled with letters
...
class Main(object):
    def __init__(self, master):
        ...
        self.canvas = Canvas(self.frame, width=int(
            GAME_SIZE / 2), height=GAME_SIZE)
        self.canvas.pack(fill=BOTH, expand=1)
        self.canvas.config(highlightbackground="yellow", highlightthickness=2)
        self.canvas_placement_queue = Canvas(self.frame, width=int(
            GAME_SIZE / 2), height=(len(SHIP_LENGTHS) * QUEUE_BLOCK_SIZE))
        self.canvas_placement_queue.pack(fill=Y)

        self.grid_block_height = 10
        self.grid_block_width = 10
        # a tuple of grid coordinates that the mouse is currently over.
        ...
        self.canvas_placement_queue.bind(
            "<Motion>", self.onPlacementQueueMouseMove)
        self.canvas_placement_queue.bind(
            "<Button-1>", self.onPlacementQueueClick)
        self.reset()
    ...
    def draw(self):
        ...
        self.grid_block_height = int(
            self.canvas.winfo_height() / 2 / GRID_SIZE)
            ...
            for y in range(int(GAME_SIZE / 2 * (p - 1)) - 1, int(GAME_SIZE / 2 * p) + 1, self.grid_block_height):
                ...
                for x in range(0, int(GAME_SIZE / 2) + 1, self.grid_block_width):
                    ...
                    self.canvas.create_rectangle(
                        x, y, x + self.grid_block_width, y + self.grid_block_height)

                    grid_pos = self.getGridPos(x, y)
                    try:
                        grid_space_content = self.getGridSpaceContent(
                            *grid_pos)
                    ...
                    if circle_color:
                        ...
                        radius = int(min(self.grid_block_width,
                                     self.grid_block_height) / 4)
                        radius = max(3, radius)
                        self.canvas.create_circle(
                            *grid_center, radius, fill=circle_color)

        # draw a thicker line in the middle between the player's boards
        self.canvas.create_line(0, int(GAME_SIZE / 2),
                                int(GAME_SIZE / 2), int(GAME_SIZE / 2), width=3)
        ...
        if self.game_phase == "setup":
            ...
            for y, boat_length in zip(range(len(self.boat_placement_queue)), self.boat_placement_queue):
                for x in range(boat_length):
                    self.canvas_placement_queue.create_circle(
                        (x * QUEUE_BLOCK_SIZE) + radius + 2, (y * QUEUE_BLOCK_SIZE) + radius + 2, radius, fill="gray")

            # indicate which boat is selected
            if self.selected_ship_index != None:
                selection_width = QUEUE_BLOCK_SIZE * \
                    self.boat_placement_queue[self.selected_ship_index]
                basey = QUEUE_BLOCK_SIZE * self.selected_ship_index
                self.canvas_placement_queue.create_rectangle(
                    0, basey, selection_width, basey + QUEUE_BLOCK_SIZE, outline="red")
    ...
    def takeTurn(self, player_num, grid_x, grid_y):
        ...
        print("Player {} {} Player {} at {}{}".format(
            attacking_player, content, player_num, GRID_LETTERS[grid_y], grid_x))
        ...
        for ship in SHIP_LENGTHS:
            while True:
                ...
                if self.getGridSpaceContent(*(2, x, y)) == "boat":
                    continue
                boat_placement, isValid = self.getShipPlacement(
                    2, (2, x, y), ship, vertical)
                if isValid:
                    for pos in boat_placement:
                        self.setGridSpaceContent(*pos, "boat")
                    break
